...
G1 の実在検査（§11.2）は**行番号 suffix を剥離してから** `existsSync` する。**カンマ区切りの複数行番号
（例 `path:12,40`）は許容しない**——`evidence_paths` のリスト構文自体がカンマ区切りのため、項目内に
カンマを含めると2項目に分解されて曖昧になる。複数行を示したい場合は行範囲（`:12-40`）を使うか、
`findings` エントリを分ける。
...
1. **`evidence_paths`（本節・上記）はバッククォート囲みを使わない**。`gates/lib/markdown.js` の
   値展開は二重引用符は剥がすがバッククォートは剥がさないため、`` `foo.md:22-33` `` は
   `existsSync('`foo.md:22-33`')` として失敗し幻覚として弾かれる。裸パスのみが正。

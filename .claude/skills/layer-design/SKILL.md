...
- **実行を要する検証（生成物に含まれるテストの実行・スクリプトの動作確認等）は、検証の主体を「オーケストレータ」
  と書く**。`designer`・`generator`・各 builder はコマンド実行系ツール（Bash 等）を持たない（G13 が
  機械強制する）ので、design-map が「generator がテストを実行して確かめる」と書いても実行できない
  （実測: run 20260919 で design-map が generator にテストの実測を要求し、結局オーケストレータが代行した）。
  ワーカーに割り当てられるのは、ファイルを読んで確かめる検証（存在・書式・件数・参照の整合）までである。

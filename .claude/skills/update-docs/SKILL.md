...
2. **`canon-update-scope-guard` は `work/.canon-update-ts` が in-flight のときのみ有効**（`/canon` 用の `write-scope-guard` とは判定材料・保護対象の極性が逆・§11.3）。`docs/` は書込対象として常に許可される。`.claude/`・`gates/`・`tests/`・`design/`（設計書2冊）は常時 deny。
3. **相互排他**: `/canon` の run（`work/.session-ts`）が in-flight のとき、`npm run canon:ts` は異常終了する。先に `/canon` run を完了させること。
4. **承認は対話で取る（承認サイドカー・`npm run approve` は廃止済み）**。更新ゲートはユーザーとの対話で採否を確定し、その内容を `work/<ts>/state.md` に記録する。**提案フェーズが `docs/` を書き換えていないこと**は、事前 deny ではなく事後照合で担保する——`npm run canon:ts` が採番時に `docs/` の sha256 スナップショット（`work/<ts>/.docs-snapshot.json`）を保存し、`canon-guard` が提案完了リクエスト（`canon-update-proposal`）の消費時に照合する。差分があれば停止をブロックする（`docs/` は git 管理下なので `git restore docs/` で採番時点へ戻せる）。
...
1. `canon-updater` を起動する（ネイティブ `subagent_type` を優先。未登録環境のみ `general-purpose`・opus・定義注入へフォールバック——後述「制約」参照。「フェーズ1（調査・差分提案）を実行せよ」と明示する）。`docs/SOURCES.md` の一次ソースを調査し、`work/<ts>/canon-diff-proposal.md` を書かせる（**`docs/` へはまだ書かせない**——書いた場合は次の手順2の事後照合が検出する）。
2. canon-updater が `work/<ts>/.requests/canon-update-proposal` を書いて完了。`canon-guard.js` はこのリクエストを G14〜G16 では検査せず、**採番時の docs スナップショットとの照合だけ**を行う（差分があれば停止をブロックし、`canon-update.blocked` を鋳造。リクエストは残して再照合させる）。それ以外の決定論ゲートは発火しない——**次の人間ゲートが実質的な関門**。
3. **更新ゲート（人間承認・唯一の関門・§13.1）**: `canon-diff-proposal.md` の絶対パスと差分候補の要約をユーザーに提示し、**採否と breaking 判定を確定してもらう**。一次ソース間の矛盾があれば併せて提示する。承認の内容（採否・breaking 判定の要旨）を `work/<ts>/state.md` に追記してから、フェーズ2へ進む。
...
- 各人間ゲート（更新ゲート）で**必ずチャット上でユーザー確認を取り停止する**。承認は対話で取り、`work/<ts>/state.md` に記録する。

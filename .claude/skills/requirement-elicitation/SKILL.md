...
- 冒頭で工程1（系統A・系統B）の調査サマリを提示し、**新規作成 / 既存改修モードをユーザーに明示選択させる**。系統B `project_profile.md` に `learning_history`（対象プロジェクト自身の学習履歴）が見つかっていれば、その要点（既知の落とし穴・過去の申し送り）もサマリに含め、要件が過去に実機で踏んだ問題を繰り返さないようヒアリングで踏まえる。

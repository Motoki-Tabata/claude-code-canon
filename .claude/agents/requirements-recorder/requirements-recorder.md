...
## 修正モード（差し戻し時）
P2 で確定要件の修正を求められたとき、オーケストレータは `work/<ts>/revisions/requirements-<n>.md`（修正後に合意した内容）を注入して本エージェントを**新規に**起動する（`SendMessage` での再開は使わない）。既存の `requirements.md` を Read し、修正指示の内容を反映して**書き直す**（Write。本エージェントは Edit を持たない）。指示に無い項目は既存の値を変えない。完了リクエストは通常どおり書く。

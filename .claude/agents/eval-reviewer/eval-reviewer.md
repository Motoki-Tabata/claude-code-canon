...
4. **集約に入る前に、judge の応答本文でなく `output/<ts>/eval/<axis>.md` の実在を Read で確認する**。
   judge が「判定した」と報告してもファイルが書かれていないことがある（実測: run 20260903_091044
   round 2 で eval-correctness が完全な verdict を応答本文に返しながら `correctness.md` を書いて
   いなかった）。5軸すべてを Read し、実在し空でないことを確かめる。
   - 欠けている軸があれば**集約を進めない**。応答本文だけを根拠に `eval-report.md` を書かないこと
     ——書いてしまうと eval-report.md は5軸そろって見えるため、人間の読みが「違反なし」へ倒れる
     （ハーネスは軸ファイルの欠落を検出するが、人間が読むのは eval-report.md である）。
   - 欠けた軸を名指しして親へ報告し、**当該 judge に書き出しをやり直させる**。判定内容が judge の
     応答に残っているなら**再判定させず、その verdict をそのまま Write させる**（再生成すると
     同じ生成物に対して round ごとに違う結論が出る）。あなたは `SendMessage` を持たないので、
     再開はオーケストレータが行う——`Agent` での再 spawn は文脈を持たない新規 judge になり
     再判定と同じなので使わない。
5. **ファイルから読んだ verdict を唯一の集約入力として**、`output/<ts>/eval-report.md` に集約する
   （下記フォーマット）。**`verdict: violation` の finding は1件残らず本文に転記する**。集約で落ちると
   P5/P7 の強制表示（§8.4）が成立しない——ハーネス `eval/report.js` がこれを機械検証する。
6. **親へ返す前に**、`output/<ts>/eval/<axis>.md` の5軸すべてと `output/<ts>/eval-report.md` を
...
- **欠けた verdict を自分で書いて埋めない**。judge の応答本文を転記するのも不可（§16.4 の判定の
  出所は judge 自身にある・手順3）。あなたがすべきは集約を止めて欠落を名指しすることだけである。

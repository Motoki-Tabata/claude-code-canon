...
## 入力（プロンプト注入）
- `output/<ts>/design-map.md` のパス（承認済み。`design.done` マーカーの存在が生成書込の前提で、advance-guard の順序ガードが強制する）。**全文は読まない**——次のスライスを読む
- `work/<ts>/slices/` の絶対パス（`npm run slice` の出力。オーケストレータが S3 の冒頭で生成済み）。`common.md`（Used Features・Model Assignments・Interface Contracts・生成上の制約）・`disposition-other.md`（keep・retire のレコード）・`l4.md`・`targets-<層>.txt`（生成される宣言パスの一覧）を使う。design-map は約96KB で、全文 Read が 1 run で 19〜30 回に達しトークン消費の主因の一つだった。スライスに無い情報が要るときだけ design-map.md の該当節を Read してよい
...
- 対象プロジェクトの非管理ファイル（`README.md`・`contracts/README.md` 等、canon の配置対象外）への参照は行番号でなく節見出しで書くよう各 Builder へ徹底する。管理ファイル間（生成物同士）の行番号参照は対象外。G7 判定⑦（詳細設計書 §11.2）が違反として検出しブロックする。
